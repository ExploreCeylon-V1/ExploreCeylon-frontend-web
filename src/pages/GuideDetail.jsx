...
const GuideDetail = () => {
  ...
  // Prefer the raw whatsappNumber (stored without '+'/spaces); fall back to phone.
  const whatsappHref = (guide?.whatsappNumber || guide?.phone)
    ? `https://wa.me/${onlyDigits(guide.whatsappNumber || guide.phone)}`
    : null;
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl p-5 sm:p-6 sticky top-4 flex flex-col gap-4">
                ...
                {isInCart('guide', guide.id) ? (
                  <button
                    type="button"
                    onClick={() => navigate('/profile?tab=cart')}
                    className="w-full bg-emerald-50 text-emerald-700 border border-emerald-200 font-semibold rounded-lg py-2.5 text-sm transition-colors"
                  >
                    ✓ In Cart — View Cart
                  </button>
                ) : (
