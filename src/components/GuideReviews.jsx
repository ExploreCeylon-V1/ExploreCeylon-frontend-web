...
/**
 * GuideReviews
 * Displays reviews for a guide and lets logged-in travelers submit a new one.
 * Mirrors GemReviews.jsx and DestinationReviews.jsx, wired to guidesService.
 *
 * Props:
 *   guideId       – the guide's ID
 *   onReviewAdded – optional callback to refresh guide data (rating/reviewCount)
 *                   after a successful submission
 */
const GuideReviews = ({ guideId, onReviewAdded }) => {
  ...
  async function fetchReviews() {
    try {
      setLoading(true);
      setError(null);
      const data = await guidesService.getGuideReviews(guideId);
      setReviews(data || []);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await guidesService.writeReview(guideId, {
        rating,
  ...
  const {
    ...
    initialCount: 5,
    increment: 5,
    resetDeps: [guideId, reviews.length],
  });
