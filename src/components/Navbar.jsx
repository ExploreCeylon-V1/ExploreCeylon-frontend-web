...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    function refresh() {
      getUnreadCount().then(c => { if (!cancelled) setUnreadCount(c); }).catch(() => {});
    }
    refresh();
    ...
    <nav className="sticky top-0 z-[1000] border-b border-gray-200 bg-white font-sans">
      ...
      <div className="flex shrink-0 items-center gap-1 sm:gap-2">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
                ...
                <div className="absolute right-0 top-[calc(100%+10px)] z-[1001] w-80 max-w-[90vw] origin-top-right overflow-hidden rounded-xl border border-gray-200 bg-white shadow-lg animate-fade-in-up">
                  <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
                    ...
                    {notifications.some(n => !n.read) && (
                      <button onClick={handleMarkAllRead} className="text-xs font-semibold text-[#2D6A4F] hover:underline">
                        Mark all read
                      </button>
                    )}
                  </div>
                      ...
                      notifications.map(n => (
                        ...
                        >
                          <div className="flex w-full items-center gap-2">
                            {!n.read && <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-[#2D6A4F]" />}
                            <span className="text-sm font-semibold text-gray-900">{n.title}</span>
                          </div>
                          <p className="text-xs text-gray-500 line-clamp-2">{n.message}</p>
                          <span className="text-3xs text-gray-400">{timeAgo(n.createdAt)}</span>
                        </button>
                      ))
                    )}
                  </div>
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              >
                ...
                <span className="hidden text-sm font-medium text-gray-900 md:inline">Hi, {user.name}</span>
                <svg
                  viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="14" height="14"
                  className={`hidden text-gray-500 transition-transform md:block ${dropdownOpen ? "rotate-180" : ""}`}
                >
                ...
                <div className="absolute right-0 top-[calc(100%+10px)] z-[1001] w-60 origin-top-right overflow-hidden rounded-xl border border-gray-200 bg-white shadow-lg animate-fade-in-up">
                  <div className="flex items-center gap-3 p-4">
                    ...
                    <div>
                      ...
                      {user.role && <span className="rounded border border-gray-300 px-1.5 py-0.5 text-3xs font-bold tracking-wider text-gray-600">{user.role}</span>}
                    </div>
                  </div>
        ...
        <button
          ...
          className="flex items-center justify-center rounded-md p-2 text-gray-600 hover:bg-gray-100 focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#2D6A4F] md:hidden"
      ...
      {/* Mobile menu panel */}
      {mobileMenuOpen && (
        <div ref={mobileMenuRef} className="border-t border-gray-100 bg-white shadow-lg animate-fade-in-up md:hidden">
